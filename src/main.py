...
def evaluate(prediction_model, extraction_model, predict_dataloader, positive_tag_idx,
    ...
    with torch.no_grad():
        for batch in predict_dataloader:
            # input_ids, attention_mask, predict_mask, label_ids = batch
            input_ids, attention_mask, label_ids, tag_ids, has_rationale = batch

            # compute outputs from the prediction module
            pred_scores = prediction_model(input_ids, attention_mask)
            # print ("shape of pred scores:", pred_scores.shape)
            pred_labels = torch.argmax(pred_scores, dim=-1)
            # print ("predictions", pred_labels)
            # print ("gold labels", label_ids)
            ...
            if 'propaganda' in args.dataset:
                # only evaluate extraction on things detected as propaganda
                is_positive = (pred_labels == 1)
                input_ids = input_ids[is_positive]
                attention_mask = attention_mask[is_positive]
                tag_ids = tag_ids[is_positive]
                pred_labels = pred_labels[is_positive]
            '''
            ...
            if len(input_ids) > 0:
                ...
                # results from the top-k attention....
                attn_pred_tags = prediction_model.get_top_tokens(input_ids, attention_mask, k=args.attention_top_k)

                attn_pred_tags_flat = [val for sublist in attn_pred_tags for val in sublist]
                attn_gold_tags_flat = (
                    torch.masked_select(tag_ids, attention_mask == 1) == positive_tag_idx
                    ).type(torch.FloatTensor).tolist()

                assert len(attn_gold_tags_flat) == len(attn_pred_tags_flat)

                top_k_attention_gold.extend(attn_gold_tags_flat)
                top_k_attention_preds.extend(attn_pred_tags_flat)

                top_k_correct_preds += np.sum(
                    np.array(attn_gold_tags_flat) == np.array(attn_pred_tags_flat))
                top_k_total_preds += len(attn_gold_tags_flat)
    ...
    # print top-k attention results...
    precision_attn, recall_attn, f1_attn, _ = precision_recall_fscore_support(
        np.array(top_k_attention_gold), np.array(top_k_attention_preds), labels=[1]
    )
    acc_top_k_attn = (1. * top_k_correct_preds) / top_k_total_preds

    end = time.time()
    print('ITER: %d | dataset: %s | Attn. Top-k: %.2f | P: %.2f | R: %.2f | F1: %.2f | T: %.3f mins' \
        % (ITER, dataset_name, 100.*acc_top_k_attn, \
            100.*precision_attn, 100.*recall_attn, 100.*f1_attn, (end-start)/60.0))
    print('--------------------------------------------------------------')
    ...
    return  {
            ...
            'f1_tag': f1_tagging,
            'acc_tag': acc_tagging,
            'p_attn': precision_attn,
            'r_attn': recall_attn,
            'f1_attn': f1_attn,
            'acc_attn': acc_top_k_attn,
        }
...
def main():
    ...
    if args.dataset == 'movie_reviews':
        ...
        dataset_processor = MultiRCProcessor()
    else:
    ...
    extraction_model = BertCRF(bert_model, start_label_id=tag_map['START'], 
                                                stop_label_id=tag_map['END'], num_labels=num_tags)
    ...
    for ITER in range(args.epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            classification_loss, output_labels = prediction_model.neg_log_likelihood(input_ids,
            ...
            attention_mask = attention_mask[has_rationale]
            tag_ids = tag_ids[has_rationale]
            output_labels = output_labels[has_rationale]


            if len(input_ids) > 0:
                extraction_loss = extraction_model.neg_log_likelihood(
                    input_ids,
                    attention_mask=attention_mask,
                    label_ids=tag_ids,
                    include_bert_features=args.include_bert_features,
        ...
        if args.extraction_coeff != 0.0:
        # print statistics from the CRF
            extraction_model.print_weights(ITER,
                args.include_avg_attention_features,
                args.include_bow_features)
